...
test("ImportDefaultSpecifier", ({ end, same }) => {
	...
	const output = `const __bar__ = require("bar"), foo = (__bar__.default || __bar__);`;
...
test("ImportSpecifier", ({ end, same }) => {
	...
	const output = `const __bar__ = require("bar"), foo = __bar__.foo, baz = __bar__.baz;`;
...
test("ImportSpecifier mixed", ({ end, same }) => {
	...
	const output = `const __bar__ = require("bar"), foo = __bar__.foo, quux = __bar__.baz;`;
...
test("import and export order", ({ end, same }) => {
	...
	const output = `const __path__ = require("path"), join = __path__.join;\nexports.default = () => {};`;
...
test("import npm namespaced modules", ({ end, same }) => {
	...
	const output = `const ___bar_baz_arr__ = require("@bar/baz-arr"), foo = (___bar_baz_arr__.default || ___bar_baz_arr__);`;
...
test("ExportNamedDeclaration single from", ({ end, same }) => {
	const input = `export { foo } from "baz"`;
	const output = `const __baz__ = require("baz"); exports.foo = __baz__.foo;`;

	same(transform(input), output);
	end();
});

test("ExportNamedDeclaration multiple from", ({ end, same }) => {
	const input = `export { foo, bar as baz } from "baz"`;
	const output = `const __baz__ = require("baz"); exports.foo = __baz__.foo; exports.baz = __baz__.bar;`;

	same(transform(input), output);
	end();
});

test("ExportNamedDeclaration default from", ({ end, same }) => {
	const input = `export { default as foo } from 'bar'`;
	const output = `const __bar__ = require("bar"); exports.foo = __bar__.default;`;

	same(transform(input), output);
	end();
});

test("ExportNamedDeclaration multiple & default from", ({ end, same }) => {
	const input = `export { baz, default as foo } from 'bar'`;
	const output = `const __bar__ = require("bar"); exports.baz = __bar__.baz; exports.foo = __bar__.default;`;
